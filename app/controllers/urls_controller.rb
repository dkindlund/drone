class UrlsController < ApplicationController
  ...
  def list_respond_to_atom
    ...
    urls = Url.find(:all, :select => 'DISTINCT urls.*, fingerprints.id AS fingerprint_id', :from => 'urls', :joins => 'LEFT JOIN fingerprints ON fingerprints.id = urls.fingerprint_id', :conditions => Url.merge_conditions(url_conditions, ['fingerprints.id IS NOT NULL'], ['urls.url_status_id IN (?,?)', UrlStatus.find_by_status("suspicious").id, UrlStatus.find_by_status("compromised").id]), :order => 'urls.time_at DESC', :limit => Configuration.find_retry(:name => "atom.max_entries", :namespace => "Url").to_i)
    fingerprints = Fingerprint.find(:all, :select => 'DISTINCT fingerprints.*, urls.id AS url_id', :from => 'urls', :joins => 'LEFT JOIN fingerprints ON fingerprints.id = urls.fingerprint_id', :conditions => Fingerprint.merge_conditions(url_conditions, ['fingerprints.id IS NOT NULL'], ['urls.url_status_id IN (?,?)', UrlStatus.find_by_status("suspicious").id, UrlStatus.find_by_status("compromised").id]), :order => 'urls.time_at DESC', :limit => Configuration.find_retry(:name => "atom.max_entries", :namespace => "Url").to_i)
