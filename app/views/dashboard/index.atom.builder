    ...
    feed.entry(url) do |entry|
      ...
      entry.content :type => 'xhtml' do |xhtml|
        ...
        if (!url.fingerprint.nil? && (url.fingerprint.os_processes.size > 0))
          ...
          related_urls_pivot_fields = [ 'urls.ip = ? OR urls.url LIKE ?', url.ip, "%" + URI.parse(url.url.strip).host().to_s + "%" ]
          joins = ''
          if !url.fingerprint.checksum.nil?
            joins = 'LEFT JOIN fingerprints ON urls.fingerprint_id = fingerprints.id'
            related_urls_pivot_fields = [ 'urls.ip = ? OR urls.url LIKE ? OR fingerprints.checksum = ?', url.ip, "%" + URI.parse(url.url.strip).host().to_s + "%", url.fingerprint.checksum.to_s ]
          end
