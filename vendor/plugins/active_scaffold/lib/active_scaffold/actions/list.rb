module ActiveScaffold::Actions
  module List
    ...
    def row
      # XXX: Added tr_class and refresh references.
      render :partial => 'list_record', :locals => {:refresh => params[:refresh], :tr_class => params[:tr_class], :record => find_if_allowed(params[:id], :read)}
    end
    ...
    def do_list
      ...
      paginate = (params[:format].nil?) ? (accepts? :html, :js) : ['html', 'js', 'atom'].include?(params[:format])
      if paginate
        options.merge!({
